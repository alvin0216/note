...
2. 为什么连接的时候是三次握手？
3. 什么是半连接队列？
4. `ISN(Initial Sequence Number)`是固定的吗？即 seq
5. 三次握手过程中可以携带数据吗？
...
:two: 第二次握手：服务器收到客户端的 `SYN` 报文之后，会以自己的 `SYN` 报文作为应答，并且也是指定了自己的初始化序列号 `ISN(s)`。同时会把客户端的 ISN + 1 作为 ACK 的值，表示自己已经收到了客户端的 SYN，此时服务器处于 `SYN_REVD` 的状态。

- 在确认报文中， `SYN=1`, 标志位 `ACK = 1`, 确认序号 `ack = X + 1`， 初始序号 `seq=y`。

:three: 第三次握手：客户端收到 SYN 报文之后，会发送一个 ACK 报文，当然，也是一样把服务器的 ISN + 1 作为 ACK 的值，表示已经收到了服务端的 SYN 报文，此时客户端处于 `ESTABLISHED` 状态。服务器收到 ACK 报文之后，也处于 `ESTABLISHED` 状态，此时，双方已建立起了连接。

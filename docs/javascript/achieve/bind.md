...
关于指定 this 的指向，我们可以使用 call 或者 apply 实现，关于 call 和 apply 的模拟实现，可以查看 [Call Apply 的模拟实现](/javascript/call-apply)。 我们来写第一版的代码：

```js
// 第一版
Function.prototype.bind2 = function(context) {
  var self = this // 存储相应的 this 对象， 比如 bar.bind2 ===> this === bar 然后在返回的函数通过 apply 绑定 this
  return function() {
...
- 如果对 `new` 不熟悉，请看 [New 的模拟实现](/javascript/new)
- 如果对原型链稍有困惑，可以查看 [JavaScript 深入之从原型到原型链](https://github.com/mqyqingfeng/Blog/issues/2)

## 构造函数效果的优化实现

但是在这个写法中，我们直接将 f`Bound.prototype = this.prototype`，我们直接修改 `fBound.prototype` 的时候，也会直接修改绑定函数的 `prototype`。这个时候，我们可以通过一个空函数来进行中转：
